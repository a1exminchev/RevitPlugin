		...
		public override void Create(revit.Document docToImport)
		{
			...
			foreach (var pair in CurveArrArray)
            {
				...
				else if (pair.Key.Split('y')[1].Split('A')[0] == numArr.ToString())
				{
					...
					{
						curveArray.Append(revit.Arc.Create(new revit.XYZ(CurveArrArray[$"Array{numArr}Arc{numLine}"][0],
																CurveArrArray[$"Array{numArr}Arc{numLine}"][1],
																CurveArrArray[$"Array{numArr}Arc{numLine}"][2]),
													new revit.XYZ(CurveArrArray[$"Array{numArr}Arc{numLine}"][3],
																CurveArrArray[$"Array{numArr}Arc{numLine}"][4],
																CurveArrArray[$"Array{numArr}Arc{numLine}"][5]),
													new revit.XYZ(CurveArrArray[$"Array{numArr}Arc{numLine}"][6],
